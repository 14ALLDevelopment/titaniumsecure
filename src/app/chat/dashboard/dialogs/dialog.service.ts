...
export class DialogService {
  ...
  //toget the ephemeral duration form current dailog
  public getEphemeralduration(dialog, currentUserId){
    if(dialog.data && dialog.data.EphemeralMessageDuration){
      let duration;
      try {
        let ephemeralDurationArr = dialog.data.EphemeralMessageDuration.map((value) => {
        ...
        ephemeralDurationArr.forEach(element => {   
          Object.keys(element).forEach(key => {
            if(key == currentUserId){
              duration =  element[key];
            }
          })
        });
